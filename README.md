...
What I didn't enjoy that much was working with changes with the articles database table. After creating the table and several articles, I wanted to replace the author's email in the articles cards with the author's name. This resulted in errors, as the existing articles did not contain this new column. I fixed this by deleting the existing articles to make sure all articles had the same setup.
